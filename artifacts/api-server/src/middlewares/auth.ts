...
export function verifyPassword(password: string, stored?: string) { if (!stored || !stored.includes(":")) return false; const [salt, hash] = stored.split(":"); if (!salt || !hash) return false; const derived = scryptSync(password, salt, 64); const saved = Buffer.from(hash, "hex"); return saved.length === derived.length && timingSafeEqual(derived, saved); }
